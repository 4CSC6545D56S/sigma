...
function parseDrawDate(dateStr) {
    if (!dateStr || typeof dateStr !== 'string') {
        console.warn('Date invalide (entrée nulle ou non-string):', dateStr);
        return new Date(); // fallback
    }

    let cleanDateStr = dateStr.trim().toLowerCase();

    // Gérer "Mercredi 26/11/2025" -> "26/11/2025"
    const partsWithSpace = cleanDateStr.split(' ');
    if (partsWithSpace.length > 1 && partsWithSpace[partsWithSpace.length - 1].includes('/')) {
        cleanDateStr = partsWithSpace[partsWithSpace.length - 1];
    }

    // Gérer "6 octobre 2008" -> "6/10/2008"
    const monthNames = {
    ...
    for (const [name, num] of Object.entries(monthNames)) {
        if (cleanDateStr.includes(name)) {
            cleanDateStr = cleanDateStr.replace(name, num).replace(/\s+/g, '/');
            break;
        }
    }
    ...
    if (parts.length !== 3) {
         console.warn('Format de date incorrect après nettoyage:', cleanDateStr, '(original:', dateStr, ')');
    ...
    if (isNaN(day) || isNaN(month) || isNaN(year)) {
        console.warn('Parties de date non valides:', { day, month, year }, 'from', dateStr);
        return new Date();
    }
    ...
    if (isNaN(date.getTime())) {
        console.warn('Date invalide après construction:', { year, month, day }, 'from', dateStr);
        return new Date();
    }
...
function weightedRandomSelection(weights, count) {
    ...
    let totalWeight = available.reduce((sum, item) => {
    ...
    for (let i = 0; i < count; i++) {
        ...
        if (totalWeight <= 0) {
            // Fallback: If no positive weights, pick a random available item
            const randomIndex = Math.floor(Math.random() * available.length);
            const selection = available.splice(randomIndex, 1)[0];
            selected.push(selection.number);
            continue;
        }
        ...
        for (let j = 0; j < available.length; j++) {
            const weight = Number.isFinite(available[j].weight) && available[j].weight > 0 ? available[j].weight : 0;
        ...
        if (selectedIndex !== -1) {
            const selection = available.splice(selectedIndex, 1)[0];
            selected.push(selection.number);
            const weight = Number.isFinite(selection.weight) && selection.weight > 0 ? selection.weight : 0;
            totalWeight -= weight;
        } else if (available.length > 0) {
            // Fallback
            const selection = available.splice(available.length - 1, 1)[0];
            selected.push(selection.number);
            const weight = Number.isFinite(selection.weight) && selection.weight > 0 ? selection.weight : 0;
            totalWeight -= weight;
        }
...
01/12/2025;11;13;14;38;46;1
29/11/25;8;13;20;23;27;3
26/11/25;9;14;16;26;33;5
24/11/25;22;28;29;36;47;1
...
function formatDate(dateStr) {                              // Déclaration fonction formatDate
    const date = parseDrawDate(dateStr);                    // Crée objet Date
...
async function runSimulation() {
    ...
    const progressText = document.getElementById('simProgressText'); // Assuming an element for progress text exists

    loading.classList.add('active');
    metricsDiv.innerHTML = '';
    resultsDiv.innerHTML = '';
    if(progressText) progressText.textContent = `Simulation en cours... 0%`;

    // Progress callback function
    const onProgress = (progress) => {
        if(progressText) progressText.textContent = `Simulation en cours... ${progress.toFixed(0)}%`;
    };

    // Use try-finally to ensure the loader is hidden
    try {
        const results = await simulateDraws(simSelectedMainNumbers, simSelectedChanceNumber, numSims, onProgress);

        metricsDiv.innerHTML = `
            <div class="stat-card"><h4>Rang 1 (5+1)</h4><div class="value">${results.rank1}</div><small>${(results.rank1 / numSims * 100).toFixed(4)}%</small></div>
            <div class="stat-card"><h4>Rang 2 (5+0)</h4><div class="value">${results.rank2}</div><small>${(results.rank2 / numSims * 100).toFixed(4)}%</small></div>
            <div class="stat-card"><h4>Rang 3 (4+1)</h4><div class="value">${results.rank3}</div><small>${(results.rank3 / numSims * 100).toFixed(4)}%</small></div>
            <div class="stat-card"><h4>Rang 4 (4+0)</h4><div class="value">${results.rank4}</div><small>${(results.rank4 / numSims * 100).toFixed(4)}%</small></div>
            <div class="stat-card"><h4>Rang 5 (3+1)</h4><div class="value">${results.rank5}</div><small>${(results.rank5 / numSims * 100).toFixed(3)}%</small></div>
            <div class="stat-card"><h4>Rang 6 (3+0)</h4><div class="value">${results.rank6}</div><small>${(results.rank6 / numSims * 100).toFixed(3)}%</small></div>
            <div class="stat-card"><h4>Rang 7 (2+1)</h4><div class="value">${results.rank7}</div><small>${(results.rank7 / numSims * 100).toFixed(3)}%</small></div>
            <div class="stat-card"><h4>Rang 8 (2+0)</h4><div class="value">${results.rank8}</div><small>${(results.rank8 / numSims * 100).toFixed(2)}%</small></div>
            <div class="stat-card"><h4>Rang 9 (Chance)</h4><div class="value">${results.rank9}</div><small>${(results.rank9 / numSims * 100).toFixed(2)}%</small></div>
            <div class="stat-card"><h4>Total Gains</h4><div class="value">${results.totalWins}</div><small>${(results.totalWins / numSims * 100).toFixed(2)}%</small></div>
        `;
        ...
        let detailedResults = `
            ...
            <div class="grid-2">
                <div class="info-box">
                    ...
                    <p><strong>Total des gains :</strong> ${results.totalWins.toLocaleString()} tirages</p>
                    <p><strong>Pourcentage de gains :</strong> ${(results.totalWins / numSims * 100).toFixed(2)}%</p>
                    <p><strong>Espérance mathématique :</strong> ${calculateExpectedValue(results, numSims).toFixed(4)} € par grille de 2.20€</p>
                </div>
                <div class="info-box">
                    <h4>Probabilités d'occurrence</h4>
                    ${Object.keys(results).filter(k => k.startsWith('rank')).map((key, i) => {
            const rank = i + 1;
            const wins = results[key];
            const probText = wins > 0 ? `1 sur ${Math.round(numSims / wins).toLocaleString()}` : `~ 0 en ${numSims.toLocaleString()} sims`;
            return `<p><strong>Rang ${rank}:</strong> ${probText}</p>`;
        }).join('')}
                </div>
            </div>
        `;
        ...
        resultsDiv.innerHTML = `<p class="error-box">Une erreur est survenue pendant la simulation.</p>`;
    } finally {
...
// Simulation de tirages
async function simulateDraws(userNumbers, userChance, numSims, onProgress) {
    return new Promise(resolve => {
        const results = {
            rank1: 0, rank2: 0, rank3: 0, rank4: 0, rank5: 0,
            rank6: 0, rank7: 0, rank8: 0, rank9: 0, totalWins: 0
        };

        // Préparer les poids pour la sélection pondérée
        const mainWeights = [];
        for (let i = 1; i <= 49; i++) {
            mainWeights.push({
                number: i,
                weight: (historicalData.frequencies[i] || 0) + 1
            });
        }
        ...
        for (let i = 1; i <= 10; i++) {
            chanceWeights.push({
                number: i,
                weight: (historicalData.chanceFrequencies[i] || 0) + 1
            });
        }

        let i = 0;
        const chunkSize = 10000; // Process 10,000 simulations per chunk

        function runChunk() {
            const limit = Math.min(i + chunkSize, numSims);
            for (; i < limit; i++) {
                // Utiliser la sélection pondérée au lieu du tirage purement aléatoire
                const drawNumbers = weightedRandomSelection(mainWeights, 5);
                const drawChance = weightedRandomSelection(chanceWeights, 1)[0];

                const matchingNumbers = userNumbers.filter(num => drawNumbers.includes(num)).length;
                const matchingChance = userChance === drawChance;

                let win = false;
                if (matchingNumbers === 5 && matchingChance) { results.rank1++; win = true; }
                else if (matchingNumbers === 5) { results.rank2++; win = true; }
                else if (matchingNumbers === 4 && matchingChance) { results.rank3++; win = true; }
                else if (matchingNumbers === 4) { results.rank4++; win = true; }
                else if (matchingNumbers === 3 && matchingChance) { results.rank5++; win = true; }
                else if (matchingNumbers === 3) { results.rank6++; win = true; }
                else if (matchingNumbers === 2 && matchingChance) { results.rank7++; win = true; }
                else if (matchingNumbers === 2) { results.rank8++; win = true; }
                else if (matchingChance) { results.rank9++; win = true; }
            ...
            if (i < numSims) {
                // Report progress and schedule the next chunk
                if (onProgress) {
                    onProgress((i / numSims) * 100);
                }
                setTimeout(runChunk, 0); // Yield to the event loop
            } else {
        ...
        runChunk(); // Start the first chunk
    });
}
...
function calculateExpectedValue(results, numSims) {
    const prizeValues = {
        ...
        rank7: 10,
        rank8: 5,
        rank9: 2.20 // Remboursement du ticket
    };

    let totalValue = 0;
    for (let i = 1; i <= 9; i++) {
        const rankKey = `rank${i}`;
        if (results[rankKey] && prizeValues[rankKey]) {
            totalValue += results[rankKey] * prizeValues[rankKey];
        }
    }
        ...
        case 'year':
            ...
            if (selectedYear) {
                filtered = filtered.filter(draw => {
                    const drawYear = parseDrawDate(draw.date).getFullYear().toString();
...
function populateYearSelectors() {
    ...
    const years = [...new Set(historicalData.draws.map(draw => {
        return parseDrawDate(draw.date).getFullYear().toString();
    }))].sort((a, b) => b - a);
...
function applyMLPeriodFilter() {
    ...
    switch (mlCurrentPeriod) {
        case 'year':
            ...
            if (selectedYear) {
                filtered = historicalData.draws.filter(draw => {
                    const drawDate = parseDrawDate(draw.date);
        ...
        default:
            ...
            cutoffDate.setMonth(cutoffDate.getMonth() - ML_DEFAULT_CONFIG.PERIOD_MONTHS);

            filtered = historicalData.draws.filter(draw =>
                parseDrawDate(draw.date) >= cutoffDate
            );

            if (filtered.length < ML_DEFAULT_CONFIG.MIN_DRAWS) {
                console.warn(`Seulement ${filtered.length} tirages trouvés sur la période de 12 mois, fallback sur les ${ML_DEFAULT_CONFIG.FALLBACK_DRAWS} derniers tirages.`);
                filtered = historicalData.draws.slice(0, ML_DEFAULT_CONFIG.FALLBACK_DRAWS);
            }
            break;
    }
    ...
    if (mlFilteredDraws.length === 0 && historicalData.draws.length > 0) {
        console.warn('Aucun tirage après filtrage, utilisation des 100 derniers tirages comme ultime fallback.');
...
// Fonction de densité de probabilité normale (pour la courbe en cloche)
...
/**
 * Calcule les combinaisons C(n, k) de manière efficace
 * @param {number} n - Nombre total d'éléments
 * @param {number} k - Nombre d'éléments à choisir
 * @returns {number} Le nombre de combinaisons
 */
function combinations(n, k) {
    if (k < 0 || k > n) return 0;
    if (k === 0 || k === n) return 1;
    if (k > n / 2) k = n - k; // Symétrie C(n, k) = C(n, n-k)
...
/**
 * Calcule la fonction de masse de probabilité (PMF) pour une loi binomiale
 * @param {number} k - Nombre de succès
 * @param {number} n - Nombre d'essais
 * @param {number} p - Probabilité de succès
 * @returns {number} La probabilité P(X=k)
 */
function binomialPMF(k, n, p) {
    if (p < 0 || p > 1 || n < 0 || k < 0 || k > n) {
        return 0;
    }
    // Gérer les cas extrêmes pour éviter les NaN de Math.pow(0, 0)
...
function dirichletPrediction(frequencies, numPredictions = 5) {
    ...
    const normalizedAlphas = {};
    for (const decade in alphas) {
        normalizedAlphas[decade] = alphas[decade] / totalAlpha;
    }

    // NOUVELLE APPROCHE : Pondération directe
    for (let pred = 0; pred < numPredictions; pred++) {
        const allNumbers = [];

        // Pondérer chaque numéro par sa dizaine ET sa fréquence
        for (const [decade, nums] of Object.entries(decades)) {
            const decadeWeight = normalizedAlphas[decade];

            nums.forEach(n => {
                allNumbers.push({
                    number: n.num,
                    weight: decadeWeight * (n.freq + 1) // Poids = proportion dizaine × fréquence
                });
            });
        }

        // Sélection pondérée unique
        const selected = weightedRandomSelection(allNumbers, 5);
        predictions.push(selected.sort((a, b) => a - b));
    }
    
    // RESTAURATION: Recréer la propriété 'interpretation'
    const interpretation = Object.entries(normalizedAlphas)
        .sort(([, a], [, b]) => b - a)
        .map(([decade, prob]) => `Dizaine ${decade}: ${(prob * 100).toFixed(1)}% de chance d'être tirée`);

    return { predictions, alphas: normalizedAlphas, interpretation };
}
...
function binomialPrediction(frequencies, numPredictions = 5) {
    ...
    const pEven = total > 0 ? totalEven / total : 0.5;
    const pOdd = 1 - pEven;

    // NOUVELLE APPROCHE : Utiliser la binomiale pour PONDÉRER les numéros
    const weights = [];
    for (let i = 1; i <= 49; i++) {
        const freq = frequencies[i] || 0;
        const isPair = (i % 2 === 0);

        // Pondération binomiale : P(succès) × fréquence historique
        const binomialWeight = isPair ? pEven : pOdd;
        const weight = binomialWeight * (freq + 1); // +1 pour éviter poids nul
...
function gammaPrediction(frequencies, numPredictions = 5) {
    const predictions = [];
    const waitTimes = {};
    const totalDraws = Math.round(Object.values(frequencies).reduce((a, b) => a + b, 0) / 5);

    for (let i = 1; i <= 49; i++) {
        const freq = frequencies[i] || 0;
        waitTimes[i] = freq > 0 ? totalDraws / freq : totalDraws * 2; // Assign a high wait time for non-appearing numbers
    }

    // NOUVELLE APPROCHE : Score basé sur l'espérance Gamma
    const weights = [];
    for (let i = 1; i <= 49; i++) {
        const freq = frequencies[i] || 0;
        ...
        // Paramètres Gamma
        const k = Math.max(1, freq); // Shape parameter alpha
        const theta = avgWait > 0 ? avgWait / k : 0; // Scale parameter beta

        // Score = Espérance Gamma = k × theta (temps d'attente optimal)
        const gammaExpectation = k * theta;

        // Inverser pour favoriser les numéros avec faible attente attendue
        const score = 1 / (1 + gammaExpectation);

        weights.push({ number: i, weight: score * 1000 });
    }
    
    // RESTAURATION: Recréer la propriété 'topNumbers'
    const topNumbers = [...weights]
        .sort((a, b) => b.weight - a.weight)
        .slice(0, 5)
        .map(item => `#${item.number} (Score: ${item.weight.toFixed(2)})`);


    // Génération
    for (let pred = 0; pred < numPredictions; pred++) {
        predictions.push(weightedRandomSelection(weights, 5));
    }
    
    const overallAvgWaitTime = Object.values(waitTimes).reduce((a, b) => a + b, 0) / 49;

    return { 
        predictions, 
        stats: { 
            avgWaitTime: overallAvgWaitTime.toFixed(2),
...
function performStatAnalysis() {
    ...
    const cltControls = document.getElementById('cltControls');
    cltControls.style.display = type === 'clt' ? 'block' : 'none';

    if (!historicalData || !filteredDraws || filteredDraws.length === 0) {
        document.getElementById('statResults').innerHTML = '<p>Données non disponibles.</p>';
        return;
    }
    
    // Détruire systématiquement l'ancien graphique
    if (charts.statChart) {
        charts.statChart.destroy();
    }
    ...
    switch (type) {
        case 'distribution':
            const freqData = Array.from({ length: 49 }, (_, i) => historicalData.frequencies[i + 1] || 0);
            title = 'Distribution des Numéros';
            config = {
                type: 'bar',
                data: {
                    labels: Array.from({ length: 49 }, (_, i) => i + 1),
                    datasets: [{ label: title, data: freqData, backgroundColor: 'rgba(102, 126, 234, 0.7)' }]
                }
            };
            break;

        case 'parite':
            const pairs = filteredDraws.reduce((sum, draw) => sum + draw.numbers.filter(n => n % 2 === 0).length, 0);
            const impairs = filteredDraws.reduce((sum, draw) => sum + draw.numbers.filter(n => n % 2 !== 0).length, 0);
            title = 'Répartition Pairs/Impairs';
            config = {
                type: 'bar',
                data: {
                    labels: ['Pairs', 'Impairs'],
                    datasets: [{ label: title, data: [pairs, impairs], backgroundColor: ['rgba(54, 162, 235, 0.7)', 'rgba(255, 99, 132, 0.7)'] }]
                }
            };
            break;

        case 'clt':
            const n = parseInt(document.getElementById('cltTrials').value) || 100;
            const p = parseFloat(document.getElementById('cltProb').value) || 0.1;
            const mu = n * p;
            const sigma = Math.sqrt(n * p * (1 - p));
            title = `Loi Binomiale B(${n}, ${p.toFixed(2)}) vs Approx. Normale`;
            resultsHTML = `<p><strong>Espérance (μ):</strong> ${mu.toFixed(2)}, <strong>Écart-type (σ):</strong> ${sigma.toFixed(2)}</p>`;
            
            if (sigma === 0) {
                 document.getElementById('statResults').innerHTML = `<h4>Résultats : ${title}</h4><p>Distribution déterministe. Écart-type nul.</p>`;
                 return; // Pas de graphique à dessiner
            }

            const binomialData = Array.from({ length: n + 1 }, (_, k) => binomialPMF(k, n, p));
            const normalData = Array.from({ length: n + 1 }, (_, k) => normalPDF(k, mu, sigma));
            ...
            config = {
                ...
                data: {
                    ...
                    datasets: [
                        {
                            type: 'bar',
                            label: 'Distribution Binomiale (Réelle)',
                            data: binomialData,
                        ...
                        {
                            type: 'line',
                            label: 'Approximation Normale (TCL)',
                            data: normalData,
        ...
        case 'somme':
            ...
            const sumBins = {};
            sums.forEach(sum => {
                const bin = Math.floor(sum / 10) * 10;
                sumBins[bin] = (sumBins[bin] || 0) + 1;
            });
            const sortedBins = Object.keys(sumBins).sort((a, b) => parseInt(a) - parseInt(b));
            title = 'Distribution des Sommes des Numéros';
            config = {
                type: 'bar',
                data: {
                    labels: sortedBins.map(bin => `${bin}-${parseInt(bin) + 9}`),
                    datasets: [{ label: title, data: sortedBins.map(bin => sumBins[bin]), backgroundColor: 'rgba(255, 159, 64, 0.7)' }]
                }
            };
            const avgSum = sums.reduce((a, b) => a + b, 0) / sums.length;
            resultsHTML = `<p><strong>Somme moyenne:</strong> ${avgSum.toFixed(2)}</p>`;
            break;
        ...
        case 'gaps':
            ...
            const sortedGaps = Object.keys(gapCounts).sort((a, b) => parseInt(a) - parseInt(b));
            title = 'Distribution des Écarts entre Numéros';
            config = {
                type: 'bar',
                data: {
                    labels: sortedGaps,
                    datasets: [{ label: title, data: sortedGaps.map(gap => gapCounts[gap]), backgroundColor: 'rgba(153, 102, 255, 0.7)' }]
                }
            };
            break;
        ...
        case 'consecutive':
            ...
            const nonConsecutiveCount = filteredDraws.length - consecutiveCount;
            title = 'Présence de Numéros Consécutifs';
            config = {
                type: 'doughnut',
                data: {
                    labels: ['Avec Consécutifs', 'Sans Consécutifs'],
                    datasets: [{ label: title, data: [consecutiveCount, nonConsecutiveCount], backgroundColor: ['rgba(255, 206, 86, 0.7)', 'rgba(75, 192, 192, 0.7)'] }]
                }
            };
            resultsHTML = `<p><strong>Tirages avec numéros consécutifs:</strong> ${consecutiveCount} (${(consecutiveCount / filteredDraws.length * 100).toFixed(1)}%)</p>`;
            break;
    }
    
    // Créer le nouveau graphique
    charts.statChart = new Chart(ctx, {
        ...config,
        options: {
            responsive: true,
            maintainAspectRatio: false,
    ...
    document.getElementById('statResults').innerHTML = `<h4>Résultats : ${title}</h4>${resultsHTML}`;
}
...
async function trainMLModel() {
    ...
    try {
        ...
        if (mlCurrentPeriod === 'all' && draws.length > 0) {
            const lastDate = parseDrawDate(draws[0].date);
            const firstDate = parseDrawDate(draws[draws.length - 1].date);
...
function createMatrixCheckboxes() {
    ...
    checkboxes.forEach(checkbox => {
        if (checkbox.id !== 'matrixHybrid') { // Ne pas mettre à jour la configuration pour le clic sur hybride lui-même
    ...
    // Logique spécifique pour la case "Modèle Hybride"
    const hybridCheckbox = document.getElementById('matrixHybrid');
    if (hybridCheckbox) {
        hybridCheckbox.addEventListener('change', function (event) {
            // Cocher/décocher toutes les autres cases
            const allCheckboxes = document.querySelectorAll('#matrice .checkbox-grid input[type="checkbox"]:not(#matrixHybrid)');
            allCheckboxes.forEach(cb => {
                cb.checked = event.target.checked;
            });
            // Mettre à jour la configuration après avoir modifié les autres cases
            updateMatrixConfiguration();
        });
    }
}
...
function getModelDisplayName(modelKey) {
    const names = {
        ...
        'chisquare': 'Test du Chi-carré',
        'student': 'Test T de Student',
...
async function trainGradientBoosting(range = "all", options = {
    ...
    // Initialiser les prédictions cumulées à 0
    let cumulativePredictions = labels.map(row => Array(49).fill(0));
    const learners = []; // Stocker les apprenants

    for (let e = 0; e < options.nEstimators; e++) {
        // Calculer les résidus DEPUIS les labels originaux
        const residuals = [];
        for (let i = 0; i < n; i++) {
            const residual = [];
            for (let j = 0; j < 49; j++) {
                residual[j] = labels[i][j] - cumulativePredictions[i][j];
            }
        ...
        // Mettre à jour les prédictions cumulées
        ...
        for (let i = 0; i < n; i++) {
            for (let j = 0; j < 49; j++) {
                const idx = i * 49 + j;
                cumulativePredictions[i][j] += options.learningRate * predArr[idx];
            }
        }
...
`;
    ...
    if (searchInput) {
        // Remplacer l'input pour être sûr de ne pas avoir d'écouteurs multiples
        const newSearchInput = searchInput.cloneNode(true);
        searchInput.parentNode.replaceChild(newSearchInput, searchInput);

        // Ajouter le nouvel écouteur pour la recherche
        newSearchInput.addEventListener('input', (e) => {
            ...
            categories.forEach(category => {
                const isCategoryVisible = Array.from(category.querySelectorAll('.search-item')).some(item => item.style.display !== 'none');
...
function runLongTermAnalysis() {
    const n = parseInt(document.getElementById('longTermPlays').value);
    const k = parseInt(document.getElementById('longTermWins').value);
    const resultsDiv = document.getElementById('longTermResults');

    // Pour un Loto standard, la probabilité de gagner n'importe quel prix est d'environ 1/6.
    // C'est une bonne approximation pour cette analyse à grande échelle.
    const p = 1 / 6;
    ...
    if (n * p < 5 || n * (1 - p) < 5) {
        resultsDiv.innerHTML = `
            <div class="warning-box">
                <p><strong>Conditions d'approximation non optimales.</strong></p>
                <p>Pour une estimation fiable avec ${n} essais, les résultats sont plus précis quand 'n*p' et 'n*(1-p)' sont supérieurs à 5. Les résultats actuels sont une indication, mais peuvent manquer de précision.</p>
            </div>`;
        return; // On peut continuer le calcul ou l'arrêter. Ici on l'arrête pour plus de rigueur.
    }

    const mu = n * p;
    const sigma = Math.sqrt(n * p * (1 - p));

    // On veut P(X >= k), ce qui est égal à 1 - P(X <= k-1).
    // On applique la correction de continuité à (k-1).
    const z = ((k - 1) + 0.5 - mu) / sigma;
    const probLessThan_k_minus_1 = normalCDF(z);
    const finalProbability = 1 - probLessThan_k_minus_1;

    // Affichage des résultats
    resultsDiv.innerHTML = `
        <h4>Estimation des chances de gain</h4>
        <p>Basé sur une probabilité de gain par grille de <strong>${(p * 100).toFixed(2)}%</strong> (approx. 1/6) :</p>
        <div class="grid-2" style="margin-top: 15px;">
            <div class="stat-card">
                <h4>Grilles jouées</h4>
                <div class="value">${n.toLocaleString()}</div>
            </div>
            <div class="stat-card">
                <h4>Gains espérés (moyenne)</h4>
                <div class="value">${mu.toFixed(1)}</div>
            </div>
        </div>
        <div class="success-box" style="margin-top: 20px;">
            <p>La probabilité d'obtenir <strong>au moins ${k} gains</strong> sur ${n} tirages est d'environ :</p>
            <div style="font-size: 2em; font-weight: bold; color: var(--accent-color); text-align: center; margin: 10px 0;">
                ${(finalProbability * 100).toFixed(4)} %
            </div>
        </div>
        <small>Analyse basée sur l'approximation par le Théorème Central Limite (Z-score = ${z.toFixed(3)}).</small>
    `;
}
